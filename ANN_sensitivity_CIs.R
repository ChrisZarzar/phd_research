...
## CZ 7-August-2017: Added an nrows=1000 parameter to read.table to force it to stop at 1000 because some of my files repeated and duplicated 1000 entries and headers at the end of the file. 
## CZ 7-August-2017: I had to remove the column names and let it dymanically add columns due to odd strings I have in some of my ANN rows, like c(3,3,3). 
...
for (d in 1:length(loop.files)){
  ...
  for (imgName in loop.imgname){
      ...
      for (nSamp in nSamp.list){
        for (size in size.list){
          for (maxIt in maxIt.list){
            start.time <- Sys.time()
            set.seed(7)
            varList <- c(paste(nSamp,',',sampRatio,',',list(size),',',maxIt, sep=''))
            xlist <- c(xlist, varList)
                ...
                #for (fname in file.names){
                  ...
                  testFile <- paste(outClassPath,classMethod,'_',nSamp,'_',sampRatio,'_',list(size),'_',maxIt,'.txt', sep='')
                  ...
                  if (firstThreeCharacters == "log"){
                    ...
                    #write(paste(classMethod,nSamp,sampRatio,list(size),maxIt,currentFile.ci[1],currentFile.ci[2],currentFile.ci[3], sep=','), file=paste(mainPath,'lpr/lpr_classification_output/',date.name,'/ANN_CIs.txt', sep=''), append = TRUE, sep = ",")
    ...
    ## Plot for ANN classifier
    outPut.cmd <- paste('png(file ="',outImgPath,'ANN_',imgName,sampRatio,'.png", width=1366, height=768)',sep='')
