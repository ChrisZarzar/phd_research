...
for (d in 1:length(loop.files)){
    ...
    for (nSamp in nSamp.list){
      for (mt in mt.list){
        for (nt in nt.list){
          start.time <- Sys.time()
          set.seed(7)
              varList <- c(paste(nSamp,',',sampRatio,',',mt,',',nt, sep=''))
              ...
              testFile <- paste(outClassPath,classMethod,'_',nSamp,'_',sampRatio,'_',mt,'_',nt,'.txt', sep='')
              ...
              if (firstThreeCharacters == "log"){
                ...
                #write(paste(classMethod,nSamp,sampRatio,mt,nt,currentFile.ci[1],currentFile.ci[2],currentFile.ci[3], sep=','), file=paste(mainPath,'lpr/lpr_classification_output/',date.name,'/RF_CIs.txt', sep=''), append = TRUE, sep = ",")
                ...
                #write(currentFile.ci, file=paste(mainPath,'lpr/lpr_classification_output/',date.name,'/SVM_CIs.txt', sep=''), append = TRUE, sep = ",")
  ...
  outPut.cmd <- paste('png(file ="',outImgPath,'RF_',imgName,sampRatio,'.png", width=1366, height=768)',sep='')
