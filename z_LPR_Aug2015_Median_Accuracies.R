...
dir.create(paste(mainPath,'lpr/lpr_classification_aug_260Samp/outAcc/',sep=''), showWarnings = FALSE)

## Set paths
outClassPath <- paste(mainPath,'lpr/lpr_classification_aug_260Samp/outAcc/',sep='')

file.names <- c('bv_3class','rad_3class','ref_3class')
for (fname in file.names){
  ## List and sort the files
  file.cmd <- paste('files <- file.info(list.files(path="',mainPath,'/lpr/lpr_classification_aug_260Samp/',fname,'", pattern="*.txt", full.names = TRUE, recursive = FALSE))', sep='')
