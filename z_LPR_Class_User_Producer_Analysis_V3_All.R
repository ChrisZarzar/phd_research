...
##EDIT: 8-Apr-2017 Chris Zarzar: Edited script to m add parameters for Random Forest
...
for (d in 1:length(loop.files)){
  ...
  for (imgName in loop.imgname){
    ...
    classMethod <- c('randomForest','logit','maximumLikelihood','neuralNetwork','supportVector') #
    nSamp.vet <- c(50, 80, 110, 140, 170, 200, 230, 260, 290, 320, 350, 380, 410, 440, 470, 500)
      ...
      for (nSamp in nSamp.vet){
        for (j in 1:1000){
          ...
          if (classTec == "logit"){
            ...
            outlist[[classTec]] <- classifyRasclass(object, splitfraction = .7, method =classTec, ntree=500, mtry=1)
          }else if (classTec =="neuralNetwork"){
            outlist[[classTec]] <- classifyRasclass(object, splitfraction = .7, method =classTec, size=c(1), maxit=100)
          }else if (classTec == "supportVector"){
            outlist[[classTec]] <- classifyRasclass(object, splitfraction = .7, method =classTec, kernel="radial", gamma=0.01, cost=1000 )
          }
          ...
          if (j==1){
            write("Class_Method,Split_Frac,Kappa,Overall_Acc,OW_UA, TV_UA, AV_UA, OW_PA, TV_PA, AV_PA", file =paste(outClassPath, classTec, '_',img.name, '_', date.name, '_',nSamp,'.txt', sep=''), append = TRUE, sep = ",")
          }
          write(paste(classTec,nSamp,kappa,overallAccuracy,accuracyMatrix[4:4],accuracyMatrix[8:8],accuracyMatrix[12:12],accuracyMatrix[13:13],accuracyMatrix[14:14],accuracyMatrix[15:15], sep =','), file =paste(outClassPath, classTec, '_',img.name, '_', date.name, '_',nSamp,'.txt', sep=''), append = TRUE, sep = ",")
