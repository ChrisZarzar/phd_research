    ...
    for (classTec in classMethod){
      ...
      for (nSamp in nSamp.vet){
        for (j in 1:1000){
          ...
          ## Classify data
          outlist <- list()
          outlist[[classTec]] <- classifyRasclass(object, splitfraction = .7, method =classTec, ntree=1000, mtry=1)
          ...
          write(paste(classTec,0.7,kappa,overallAccuracy,accuracyMatrix[4:4],accuracyMatrix[8:8],accuracyMatrix[12:12],accuracyMatrix[13:13],accuracyMatrix[14:14],accuracyMatrix[15:15], sep =','), file =paste(outClassPath, classTec, '_',img.name, '_', date.name, '_', nSamp,'.txt', sep=''), append = TRUE, sep = ",")
