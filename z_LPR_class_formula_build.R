##z_LPR_class_formula_build.R

##Author: Christopher Zarzar
##NOTES: This script takes the 6000 training samples from an August 2015 mosaic over the 
##Lower Pearl River and stores the classification formula for application to new data
...
# rad.data <- read.csv("/home/chriszarzar/Desktop/RWorkspace/datasets/aug2015Rad25_3class.csv", header = TRUE)
...
for (k in 1:length(classMethod)){
  ...
  if (classMethod[k] == "maximumLikelihood"){
    mlc.bv.predict <- predict(outlist$maximumLikelihood@maximumLikelihood)
  }
  if (classMethod[k] == "logit"){
    log.bv.predict <- predict(outlist$logit@logit)
  }
  FINISH THIS WITH THE OTHER CLASSESS AND COPY TO OTHER IMAGE TYPES BELOW ONCE I GET BACK TO IT
  This is not really necessary and is a waste of time right now, so I am going to work soley on 
  the mosaic classification and I can come back to this if I find that I need it. 
}
closeAllConnections()
...
for (k in 1:length(classMethod)){
  ...
  outlist <- list()
  outlist[[classMethod[k]]] <- classifyRasclass(object, splitfraction = .7, method =classMethod[k])
  y.hat <- predict(outlist$supportVector@supportVector,augbv.data)
  kappa <- outlist[[classMethod[k]]]@kappa
  overallAccuracy <- outlist[[classMethod[k]]]@overallAccuracy
  accuracyMatrix <- outlist[[classMethod[k]]]@accuracyMatrix
